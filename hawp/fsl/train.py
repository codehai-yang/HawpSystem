...
def train(cfg, model, train_dataset, optimizer, scheduler, loss_reducer, checkpointer, arguments):
    ...
    for epoch in range(start_epoch+1, start_epoch+num_epochs+1):
        ...
        scheduler.step()
        if not hasattr(arguments, 'best_loss'):
            arguments['best_loss'] = float('inf')
        current_loss = loss_meters.loss.global_avg
        if current_loss < arguments['best_loss']:
            arguments['best_loss'] = current_loss
            checkpointer.save('model_best')
            logger.info(f'保存最优模型，loss={current_loss:.4f}')
